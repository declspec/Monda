...
namespace Monda {
    public readonly ref struct ParseResult<TValue> {
        ...
        public ParseResult(TValue value, int start, int length) {
            Value = value;
            Start = start;
            Length = length;
        }
    ...
    public static class ParseResult {
        ...
        public static ParseResult<T> Fail<T>(T value) => new ParseResult<T>(value, -1, -1);

        public static ParseResult<T> Success<T>(T value, int start, int length) {
